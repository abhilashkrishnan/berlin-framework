...
Invocation of methods across disparate languages in the same container work without the use of RPC. It will work "as a" `native method call` (while deployed in the same container) without the use of any cumbersome or additional configuration and hence it will be much faster compared to RPC which involves the overhead of `marshalling and unmarshalling` of objects. Hence we try to avoid it all costs at the same time providing deployment of `polygot modules` in the same container and eventually invoke the methods as `native method calls`. And that's `secret sauce` of Berlin Framework! 
